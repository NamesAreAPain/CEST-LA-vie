...
class DataPoint :
    ...
    def __mul__(self,other):
        if isinstance(other, DataPoint):
            return DataPoint(0,0,override=True,val=self.value*other.value,err=abs(self.value*other.value)*sqrt((self.error/self.value)**2+(other.error/other.value)**2))
        else:
    ...
    def __pow__(self,other):
        ERR = lambda a,b : abs(a.value**other)*abs(other)*self.error/abs(self.value)
        return DataPoint(0,0,override=True,val=self.value**other,err=ERR(self,error))
